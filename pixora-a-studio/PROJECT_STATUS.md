...
БАГ 6 — резиновый холст (canvas ломается при ресайзе)
При уменьшении окна браузера .stage (рамка) сжимается,
но canvas внутри остаётся фиксированного размера —
рисунок вылезает за пределы stage.
Нужно: холст должен масштабироваться вместе со stage,
либо stage не должен быть меньше холста (overflow + scroll).
