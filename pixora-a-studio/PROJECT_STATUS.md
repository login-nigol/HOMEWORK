...
1 — боковое меню уезжает при выборе слоя, отключить,
закрывается только если нажать на тугл или на пустом экране
2 — при нажатии на пустой экран при открытой панели, срабатывает карандаш,
нужно запретить первый раз пир клики на холст

3 — холст прижат к левому верхнему углу, нужно центрировать
стек центрировать в стейдже
6 — резиновый холст (canvas ломается при ресайзе)
...
6 — ластик на image-слоях, не сохраняет изменения после трансормации картинки,
(большой рефакторинг — после сдачи)
